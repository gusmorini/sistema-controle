...
class Usuarios extends CI_Controller {
	...
	public function __construct(){
		...
		$this->load->model('usuarios_model','modelusuario');		
	}
	...
	public function index(){
		...
		$dados['usuarios'] = $this->modelusuario->listar_autores();
	...
	public function inserir(){
		...
		$this->form_validation->set_rules('txt-historico','Histórico','required|min_length[3]');
		...
		$this->form_validation->set_rules('txt-confir-senha','Conferir Senha','required|matches[txt-senha]');
		if($this->form_validation->run()==FALSE){
			...
			if($this->modelusuario->adicionar($nome,$email,$historico,$user,$senha)){
	...
	public function excluir($id){
		...
		if($this->modelusuario->excluir($id)){
			redirect(base_url('admin/usuarios'));
		}else{
			echo "Houve um erro no sistema!";
		}		
	...
	public function alterar($id){
		...
		$dados['usuarios'] = $this->modelusuario->listar_usuario($id);
		//cabeçalho dinâmico
		$dados['titulo'] = "Painel de controle";
		$dados['subtitulo'] = "Usuários";
		$this->load->view('backend/template/html-header',$dados);
	...
	public function salvar_alteracoes(){
		...
		if($this->form_validation->run()==FALSE){
			...
			//$user = $this->input->post('txt-user');
			$senha = $this->input->post('txt-senha');
			if($this->modelusuario->alterar($id,$nome,$email,$historico,$senha)){
				redirect(base_url('admin/usuarios'));
			}else{
				echo "Houve um erro no sistema!";
			}
	...
	public function nova_foto(){
		...
		if(!$this->upload->do_upload()){
			...
			if($this->image_lib->resize()){
				if($this->modelusuario->alterar_img($id)){
